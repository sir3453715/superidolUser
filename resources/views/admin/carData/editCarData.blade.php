...
@section('admin-page-content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">修改隔熱紙紀錄 會員 : {{$user->name}}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">修改隔熱紙紀錄</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    ...
    <form id="admin-form" class="admin-form" action="{{ route('admin.carData.update',['carDatum'=>$carData->id]) }}" method="post">
        @csrf
        @method('PUT')
        <!-- Main content -->
                            ...
                            <div class="card-body">
                                <div class="form-group row">
                                    <div class="form-group col-md-3">
                                        ...
                                        <input type="date" class="form-control form-required" name="date" id="date" value="{{$carData->date}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="time">來電時間*</label>
                                        <input type="time" class="form-control form-required" name="time" id="time" value="{{ date('H:i',strtotime($carData->time)) }}">
                                    </div>
                                    <div class="form-group col-md-3">
                                            ...
                                            @foreach($carTypes as $carType)
                                                <optgroup label="{{$carType->name}}">
                                                    @foreach($carType->childrenType() as $children)
                                                        <option value="{{$children->id}}" {!! $html->selectSelected($children->id, $carData->car_type) !!}>{{$carType->name}} {{$children->name}}</option>
                                                    @endforeach
                                                </optgroup>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <select class="form-control select2" id="car_model" name="car_model">
                                            ...
                                            @foreach($carModels as $carModel)
                                                <option value="{{$carModel->id}}" {!! $html->selectSelected($carModel->id, $carData->car_model) !!}>{{$carModel->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="VIN">車輛識別號碼</label>
                                        <input type="text" id="VIN" class="form-control" name="VIN" value="{{$carData->VIN}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="car_code">車牌號碼*</label>
                                        <input type="text" id="car_code" class="form-control form-required" name="car_code" value="{{$carData->car_code}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="milage">里程數</label>
                                        <input type="text" id="milage" class="form-control" name="milage" value="{{$carData->milage}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="price">價格</label>
                                        <input type="text" id="price" class="form-control" name="price" value="{{$carData->price}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="giveaway">贈品</label>
                                        <textarea class="form-control" id="giveaway" name="giveaway" rows="3">{!! $carData->giveaway !!}</textarea>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="how_to_know">如何得知超級名膜</label>
                                        <textarea class="form-control" id="how_to_know" name="how_to_know" rows="3">{!! $carData->how_to_know !!}</textarea>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="car_situation">車輛狀況</label>
                                        <textarea class="form-control" id="car_situation" name="car_situation" rows="3">{!! $carData->car_situation !!}</textarea>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="notes">備註</label>
                                        <textarea class="form-control" id="notes" name="notes" rows="3">{!! $carData->notes !!}</textarea>
                                    </div>
                                </div>
                            </div>
                            <!-- /.card-body -->
                                ...
                                @foreach($carData->CarItems as $items)
                                    <div class="form-group row border-bottom item_wrapper">
                                        <div class="form-group col-md-3">
                                            ...
                                            <input type="text" class="form-control form-required" name="location[]" id="location" value="{{$items->location}}">
                                        </div>
                                        <div class="form-group col-md-3">
                                            <label for="paper_type">隔熱紙型號</label>
                                            <input type="text" class="form-control form-required" name="paper_type[]" id="paper_type" value="{{$items->paper_type}}">
                                        </div>
                                        <div class="form-group col-md-3">
